      ...
      <div className="flex-1 ml-64">
        ...
        <main className="mt-[72px] p-6 space-y-5">
          ...
          <div className="animate-fade-up">
            ...
            <p className="text-sm text-gray-400 mt-0.5">View and manage all admin accounts</p>
          </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden animate-fade-up">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-50">
                  {admins.map(a => (
                    <tr key={a.id} onClick={() => setSelected(a)} className="hover:bg-steelblue-50/60 cursor-pointer transition-all duration-150">
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-full bg-steelblue-100 flex items-center justify-center flex-shrink-0">
                            <span className="text-xs font-bold text-steelblue-600">{(a.name || '?')[0].toUpperCase()}</span>
                          </div>
                          <span className="text-sm font-semibold text-gray-800">{a.name}</span>
                        </div>
                      </td>
                      <td className="px-5 py-4 text-sm text-gray-500">{a.email}</td>
                      <td className="px-5 py-4 text-sm text-gray-500">{a.phone}</td>
                      <td className="px-5 py-4">
                        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium rounded-full ${
                          a.status === 'Active' ? 'bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200' : 'bg-gray-100 text-gray-500 ring-1 ring-gray-200'
                        }`}>
                          <span className={`w-1.5 h-1.5 rounded-full ${a.status === 'Active' ? 'bg-emerald-500' : 'bg-gray-400'}`} />
                          {a.status}
                        </span>
                      </td>
                      <td className="px-5 py-4 text-sm text-gray-500">{a.createdAt ? new Date(a.createdAt).toLocaleDateString() : '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="fixed inset-0 bg-black/30 backdrop-blur-[2px] flex items-center justify-center z-50 p-4 animate-fade-in"
          ...
          <div className="bg-white rounded-3xl w-full max-w-md max-h-[92vh] flex flex-col shadow-2xl animate-scale-in">
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
              <h2 className="text-base font-bold text-gray-900">Admin Details</h2>
            ...
            <div className="overflow-y-auto flex-1 px-6 py-5 space-y-4">
              <div className="flex items-center gap-4 bg-steelblue-50 rounded-2xl p-4">
                <div className="w-12 h-12 bg-steelblue-200 rounded-full flex items-center justify-center text-steelblue-700 text-lg font-bold flex-shrink-0">
                  {selected.name?.[0] || 'A'}
                </div>
                <div>
              ...
              <div className="grid grid-cols-2 gap-2.5">
                {[['Phone', selected.phone], ['Role', selected.role || 'Admin'], ['Created', selected.createdAt ? new Date(selected.createdAt).toLocaleDateString() : '—'], ['Status', selected.status]].map(([label, val]) => (
