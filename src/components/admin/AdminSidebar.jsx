  ...
  return (
    <aside className="w-64 bg-white min-h-screen fixed left-0 top-0 border-r border-gray-200 flex flex-col animate-slide-left">
      ...
      <nav className="p-4 space-y-1 flex-1">
        {menuItems.map((item) => (
          <Link key={item.path} to={item.path} className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${location.pathname === item.path ? 'bg-steelblue-50 text-steelblue-500' : 'text-gray-700 hover:bg-gray-50'}`}>
            {item.icon}
      ...
      <div className="p-4 border-t border-gray-200 relative" ref={dropRef}>
        ...
        {dropOpen && (
          <div className="absolute bottom-full left-4 right-4 mb-2 bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden z-50 animate-fade-up">
            ...
            <Link to="/admin/settings" onClick={() => setDropOpen(false)} className="flex items-center gap-3 px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 transition-colors">
