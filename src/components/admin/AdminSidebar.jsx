  ...
  return (
    <aside className="w-64 bg-white min-h-screen fixed left-0 top-0 border-r border-gray-200 flex flex-col">
      ...
      <nav className="p-4 space-y-1 flex-1">
        {menuItems.map((item) => (
          <Link key={item.path} to={item.path} className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${location.pathname === item.path ? 'bg-sky-50 text-sky-600' : 'text-gray-700 hover:bg-gray-50'}`}>
            {item.icon}
      ...
      <div className="p-4 border-t border-gray-200 relative" ref={dropRef}>
        ...
        >
          <div className="w-9 h-9 bg-sky-600 rounded-full flex items-center justify-center text-white font-semibold text-sm flex-shrink-0">
            {user?.name?.[0] || 'A'}
          </div>
