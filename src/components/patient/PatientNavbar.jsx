...
const STORAGE_KEY = (uid) => `patient_notif_read_${uid}`;
          ...
          <div className="relative">
            ...
            {open && (
              <div className="absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-lg border border-gray-100 z-50 animate-fade-down">
                <div className="px-4 py-3 font-semibold text-gray-700 text-sm">Notifications</div>
                    ...
                    {notifications.map(n => (
                      <li key={n.id} className="px-4 py-3 hover:bg-gray-50">
                        <p className="text-sm font-medium text-gray-800">
                          Appointment {n.status === 'Confirmed' ? '✅ Confirmed' : '🎉 Completed'}
                        </p>
                        <p className="text-xs text-gray-500 mt-0.5">{n.specialty} — {n.date} at {n.time}</p>
                      </li>
                    ))}
                  </ul>
                )}
